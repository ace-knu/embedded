...
void initCCU60(void)
{
    ...
    CCU60_TCTR0 |=  ((0x02) << T12CLK);     // f_CCU6 = 50 MHz, prescaler = 4
    //CCU60_TCTR0 |= ((0x01) << T12PRE);    // f_T12 --> 12.5 MHz

    CCU60_TCTR0 &= ~((0x01) << CTM);        // T12 auto reset when period match (PM) occur

    CCU60_T12PR = 125 - 1;                  // PM interrupt freq. = f_T12 / (T12PR + 1)
...
void initCCU61(void)
{
    ...
    CCU61_TCTR0 &= ~(0x7 << T12CLK);        // f_T12 = f_CCU6 / prescaler = 12.5 MHz
    CCU61_TCTR0 |= 0x2 << T12CLK;           // f_CCU6 = 50 MHz, prescaler = 4

    CCU61_TCTR0 |= 0x1 << T12PRE;           // f_T12 = f_CCU6 / 256 = 48,828 Hz
