...
void init_GTM_TOM0_PWM(void)
{
    ...
    GTM_TOM0_TGC0_GLB_CTRL |= ((0x2) << UPEN_CTRL5);    // TOM channel 5 enable update of
                                                        // register CM0, CM1, CLK_SRC

    GTM_TOM0_TGC0_FUPD_CTRL |= ((0x2) << FUPD_CTRL1);   // Enable force update of TOM channel 1
    GTM_TOM0_TGC0_FUPD_CTRL |= ((0x2) << RSTCN0_CH1);   // Reset CN0 of TOM channel 1 on force update
    ...
    GTM_TOM0_CH1_CTRL &= ~((0x7) << CLK_SRC_SR);        // Clock source : CMU_FXCLK(1) = 3125 kHz
    GTM_TOM0_CH1_CTRL |= (1 << CLK_SRC_SR);
    GTM_TOM0_CH1_SR0 = 12500 - 1;                       // PWM freq. = 3125 kHz / 12500 = 250 Hz
